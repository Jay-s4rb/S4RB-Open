    ...
    for(let date of m) {
      for(let obj of e){
        if(fmt(date) === fmt(obj.Month)){
          existing.push({Quarter: obj.Quarter, Month: fmt(date), Cpmu: cpmuCalc(obj.Complaints, obj.UnitsSold).toFixed(8)});
        }
      }
...
export { cpmuCalc as calc, fmt, groupQuarterly, cpmuQuarterly, fillDates, allDates };
